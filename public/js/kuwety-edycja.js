...
document.addEventListener("DOMContentLoaded", async () => {
  const smak = document.getElementById("dodawanie-kuwet-smak");
  const rozmiar = document.getElementById("dodawanie-kuwet-rozmiar");
  const sklep = document.getElementById("dodawanie-kuwet-sklep");

  const urlParams = new URLSearchParams(window.location.search);
  const kuwetaId = urlParams.get("kuweta");

  if (!kuwetaId) {
    console.error("Brak ID kuwety w URL!");
    return;
  }

  try {
    const response = await fetch(`${CONFIG.URL}/api/kuwety-edycja/${kuwetaId}`);
    if (!response.ok) {
      throw new Error("Błąd podczas pobierania danych kuwety");
    }

    const kuwetaArray = await response.json();
    const kuweta = kuwetaArray[0];

    smak.value = kuweta.KuwSmkId;
    rozmiar.value = kuweta.KuwRozId;
    sklep.value = kuweta.KuwSklId || "";
  } catch (error) {
  ...
  document
    ...
    .addEventListener("submit", async function (event) {
      ...
      const smak = document.getElementById("dodawanie-kuwet-smak").value;
      const rozmiar = document.getElementById("dodawanie-kuwet-rozmiar").value;
      const sklep = document.getElementById("dodawanie-kuwet-sklep").value;
      const messageElement = document.getElementById("message");
      const messageContainer = document.querySelector(".message");

      try {
        const response = await fetch(
          `${CONFIG.URL}/api/kuwety-edycja-zapis/${kuwetaId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify({ smak, rozmiar, sklep }),
          }
        );
        ...
        if (response.ok) {
          ...
          messageElement.textContent =
            "Wystąpił błąd podczas edytowania kuwety";
        ...
        messageElement.textContent = "Wystąpił błąd podczas edytowania kuwety";
        messageContainer.style.opacity = 1;
        setTimeout(() => {
          messageContainer.style.opacity = 0;
        }, 3000);
